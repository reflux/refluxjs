...
* `preEmit` - Is called before the action emits an event. It receives the arguments from the action invocation. If it returns an array, that will be used as arguments for `shouldEmit` and `trigger`.
